...
def get_oci_price_ords(sku):
    ...
    return pay_as_you_go, monthly_commit
...
def Block_volume_GB_per_month(block_storage_price, storage_capacity, vpus_ocpu_price, vpus_per_gb):
    ...
    Monthly_Flex_Block_vpus = vpus_ocpu_price[1] * \
        vpus_per_gb * storage_capacity

    PAYG_Block_Volume_price = round(PAYG_Block_Storage + PAYG_Block_vpus, 2)
    Monthly_Flex_Block_Volume_price = round(
        Monthly_Flex_Block_Storage + Monthly_Flex_Block_vpus, 2)
...
def BM_Exa_OCPU_per_hr(license_model, database_edition, dbaas_license_price, dbaas_shape, additional_ocpu):
    ...
    if dbaas_shape == "bm":
        ...
        PAYG_OCPU += round(additional_dense_ocpu_price[0]
                           * additional_ocpu * ocpu_per_month, 2)
        # Monthly Flex for additonal ocpu
        Monthly_Flex_OCPU += round(
            additional_dense_ocpu_price[1] * additional_ocpu * ocpu_per_month, 2)
        ...
        # PAYG for additonal ocpu
        PAYG_OCPU += round(additional_exacs_ocpu_price[0]
                           * additional_ocpu * ocpu_per_month, 2)
        # Monthly Flex for additonal ocpu
        Monthly_Flex_OCPU += round(
            additional_exacs_ocpu_price[1] * additional_ocpu * ocpu_per_month, 2)
    ...
    if license_model == "LICENSE_INCLUDED":
        if dbaas_shape == "vm":
            if edition == "standard_edition":
                PAYG_DBaaS, Monthly_Flex_DBaaS = get_oci_price_ords("B90569")
            elif edition == "enterprise_edition":
                PAYG_DBaaS, Monthly_Flex_DBaaS = get_oci_price_ords("B90570")
            elif edition == "enterprise_edition_high_performance":
                PAYG_DBaaS, Monthly_Flex_DBaaS = get_oci_price_ords("B90571")
            elif edition == "enterprise_edition_extreme_performance":
                PAYG_DBaaS, Monthly_Flex_DBaaS = get_oci_price_ords("B90572")
        elif dbaas_shape == "bm":
            if edition == "standard_edition":
                PAYG_DBaaS, Monthly_Flex_DBaaS = get_oci_price_ords("B89621")
            elif edition == "enterprise_edition":
                PAYG_DBaaS, Monthly_Flex_DBaaS = get_oci_price_ords("B89622")
            elif edition == "enterprise_edition_high_performance":
                PAYG_DBaaS, Monthly_Flex_DBaaS = get_oci_price_ords("B89623")
            elif edition == "enterprise_edition_extreme_performance":
                PAYG_DBaaS, Monthly_Flex_DBaaS = get_oci_price_ords("B89624")
    elif license_model == "BRING_YOUR_OWN_LICENSE":
        if dbaas_shape == "vm":
            PAYG_DBaaS, Monthly_Flex_DBaaS = get_oci_price_ords("B90573")
        elif dbaas_shape == "bm":
            PAYG_DBaaS, Monthly_Flex_DBaaS = get_oci_price_ords("B89625")

    return PAYG_DBaaS, Monthly_Flex_DBaaS
